...
зачисленията: в crm_audit_log, БЕЗ отделна таблица — решение на потребителя)
...
134 теста (`npm test`, vitest) — чиста логика: `contract`, `utils`, `trz`,
`tableIndices`, `subscriptionBuckets`, `monitoring`, `notifications`, `certificates`. НЯМА тестове на компоненти
