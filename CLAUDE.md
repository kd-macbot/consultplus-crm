...
- ⬜ **Resend + DNS за известията** — акаунт в Resend, верификация на
  `cplus360.com` (SPF/DKIM/DMARC записи в Cloudflare), API ключ. Без
  верифицирания домейн писмата отиват в спам. Стъпките са в
  `supabase/NOTIFICATIONS.md`.
- ⬜ **Secrets за `mail-send`** — `RESEND_API_KEY`, `MAIL_FROM`,
  `NOTIFY_CRON_SECRET` на PROD проекта (dev е ОТДЕЛЕН проект — иска си свои).
- ⬜ **GitHub secrets за графика** — `SUPABASE_FUNCTIONS_URL`,
  `NOTIFY_CRON_SECRET` (същият низ).
