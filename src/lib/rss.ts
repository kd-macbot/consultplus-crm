...
export function extractListing(html: string, base: string): FeedItem[] {
  ...
  // 1) Под ПЪЛНАТА пътека на раздела — най-точното попадение.
  let best = anchors.filter(a => {
    const p = pathOf(a.link)
    return basePath.length > 1 && p.startsWith(basePath + '/')
  })

  // 2) Иначе под същата първа част („/novini") — разделът може да сочи
  //    към статии, вдигнати едно ниво нагоре.
  if (best.length < MIN_GROUP && baseGroup.length > 1) {
    best = anchors.filter(a => pathGroup(a.link) === baseGroup)
  }

  // 3) Чак накрая — най-многолюдната група. Работи за начални страници,
  //    където подаденият адрес не носи информация.
