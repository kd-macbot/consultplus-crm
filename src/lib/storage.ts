...
export async function trackSave<T>(p: Promise<T>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => {
      // Записът виси → stale HTTP/2 връзка. Само page reload дава свежа
      // връзка (както при четенето). attemptAutoReload има burst guard,
      // а формите с draft persistence (Банков достъп) възстановяват
      // въведеното след reload-а, така че нищо не се губи.
      attemptAutoReload('trackSave timeout (15s hang)')
      reject(new Error(SAVE_HANG_MSG))
    }, SAVE_TIMEOUT_MS)
    p.then(
      v => { clearTimeout(timer); resolve(v) },
      e => {
        clearTimeout(timer)
        // Fetch-ът се отмени по timeout (10s) преди SAVE_TIMEOUT → пак е
        // stale връзка, пак пробваме recovery.
        if (isTimeoutError(e)) attemptAutoReload('trackSave fetch timeout')
        reject(e)
      },
    )
  })
...
export async function getTasks(): Promise<Task[]> {
  return withRetry(async () => {
    const { data, error } = await supabase
      .from('crm_tasks')
      .select('id,title,description,status,kind,inspection_type,inspector_name,inspector_phone,inspector_email,documents_url,assignee_staff_id,client_id,due_date,position,created_by,created_at,updated_at')
      .order('position')
    if (error) throw error
    return (data ?? []) as Task[]
  })
...
export async function addTask(
  patch: Pick<Task, 'title'> & Partial<Pick<Task, 'description' | 'status' | 'kind' | 'inspection_type' | 'inspector_name' | 'inspector_phone' | 'inspector_email' | 'documents_url' | 'assignee_staff_id' | 'client_id' | 'due_date'>>,
  createdBy?: string | null,
): Promise<Task> {
  return await trackSave((async () => {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        inspector_email: patch.inspector_email ?? null,
...
export async function updateTask(
  id: string,
  patch: Partial<Pick<Task, 'title' | 'description' | 'status' | 'inspection_type' | 'inspector_name' | 'inspector_phone' | 'inspector_email' | 'documents_url' | 'assignee_staff_id' | 'client_id' | 'due_date' | 'position'>>,
): Promise<void> {
  await trackSave((async () => {
    const { error } = await supabase
      .from('crm_tasks')
