...
export async function getTasks(): Promise<Task[]> {
  return withRetry(async () => {
    const { data, error } = await supabase
      .from('crm_tasks')
      .select('id,title,description,status,assignee_staff_id,client_id,due_date,position,created_by,created_at,updated_at')
      .order('position')
    if (error) throw error
    return (data ?? []) as Task[]
  })
...
export async function addTask(
  patch: Pick<Task, 'title'> & Partial<Pick<Task, 'description' | 'status' | 'assignee_staff_id' | 'client_id' | 'due_date'>>,
  createdBy?: string | null,
): Promise<Task> {
  return await trackSave((async () => {
    const { data, error } = await supabase
...
export async function updateTask(
  id: string,
  patch: Partial<Pick<Task, 'title' | 'description' | 'status' | 'assignee_staff_id' | 'client_id' | 'due_date' | 'position'>>,
): Promise<void> {
  await trackSave((async () => {
    const { error } = await supabase
      .from('crm_tasks')
